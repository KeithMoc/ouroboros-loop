...
class TestExecuteSeedHandlerSubagentDispatch:
    ...
    async def test_plugin_payload_includes_resolved_worker_cap(self, handler) -> None:
        ...
        Q4.1 / AC-2 reordered seed-parse before plugin-dispatch so the seed
        contract is honored on delegated runs too — therefore this test
        passes a fully-validated seed YAML, not the pre-Q4.1 ``goal: test``
        shorthand which fails Pydantic validation before dispatch.
        """
        ...
        with patch(
            ...
            result = await handler.handle({"seed_content": _VALID_SEED_YAML})
        ctx = result.value.meta["_subagent"]["context"]
